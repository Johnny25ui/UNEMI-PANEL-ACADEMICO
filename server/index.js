...
async function syncActivity(x) {
  ...
  if (!activity.course || !activity.title || !activity.url) {
    throw new Error('course, title y url son obligatorios para sincronizar');
  }

  if (!pool) {
    const items = readLocal();
    const index = items.findIndex(a => a.url && a.url === activity.url);

    if (index >= 0) {
      // Conservamos el estado elegido manualmente en el panel.
      items[index] = {
        ...items[index],
...
const headers = {
  ...
  'Access-Control-Allow-Headers':
    'Content-Type, X-Admin-Key, X-Sync-Key'
};
...
const server = http.createServer(
  async (req, res) => {
    ...
    try {
      ...
      /* =====================================
         SINCRONIZACION DESDE EXTENSION UNEMI
      ===================================== */

      if (
        req.url === '/api/sync/activity' &&
        req.method === 'POST'
      ) {
        if (!isSync(req)) {
          return send(res, 403, {
            error: 'Clave de sincronización incorrecta'
          });
        }

        const x = await body(req);

        if (!x.course || !x.title || !x.url) {
          return send(res, 400, {
            error: 'course, title y url son obligatorios'
          });
        }

        const result = await syncActivity(x);
        return send(res, result.added ? 201 : 200, result);
      }
...
async function start() {
  ...
  try {
    ...
    console.log(
      SYNC_KEY
        ? 'Sincronizacion: SYNC_KEY configurada.'
        : 'Sincronizacion: SYNC_KEY NO configurada.'
    );
